...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        output.setText(coreTime(vspishka, delayMinute, volumeData, coreFrequency) + " секунд");
    }
    ...
    private int volumeTime(int coreFrequency, int volumeData, int vspishka) {

        return ((volumeData * vspishka) / coreFrequency) / 60;
    ...
    private int coreTime(int[] delayMinute, int[] volumeData, int[] vspishka, int frequency) {
        ...
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeData[i], vspishka[i]); // определение времени обработки компьютером данных от одного спутника
        }
